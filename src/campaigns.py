...
from opencti import get_actor_details, get_actor_techniques, get_sigma_rules, get_sigma_techniques, update_rules_in_opencti, remove_disabled_coas, get_actor_campaigns
...
def main():
    ...
    all_actors = []
    missing_actors = []
    log_info(f"Processing TOP {TABLE_LENGTH} systems: {CyABS}")
    for actor in CyABS:
    ...
    log_info(f"Processing campaigns for {len(all_actors)} actors.")
    campaigns = []
    for actor in all_actors:
        actor_campaigns = get_actor_campaigns(OPENCTI_URL, OPENCTI_HEADERS, actor)
...
The Organisation has prioritised the following actors; **{', '.join(CyABS)}** however there is no details in OpenCTI on **{', '.join(missing_actors)}**. The report provides an overview of available actors techniques, coverage by Elastic rules and suggested Sigma rules.
...
"""
    ...
    report_title = datetime.now().strftime("%Y-%m-%d-CyAB-Report.md")
